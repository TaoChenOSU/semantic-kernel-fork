...
export const useChat = () => {
    ...
    const createChat = async () => {
        ...
        try {
            ...
            await sk.invokeAsync(ask, 'ChatMemorySkill', 'CreateChat', await AuthHelper.getSKaSAccessToken(instance)).then(async (result: IAskResult) => {
    ...
    const getResponse = async (value: string, chatId: string) => {
        ...
        try {
            var result = await sk.invokeAsync(ask, 'ChatSkill', 'Chat', await AuthHelper.getSKaSAccessToken(instance));
    ...
    const loadChats = async () => {
        try {
            ...
            var result = await sk.invokeAsync(ask, 'ChatMemorySkill', 'GetAllChats', await AuthHelper.getSKaSAccessToken(instance));
            ...
            if (Object.keys(chats).length > 0) {
                ...
                for (const index in chats) {
                    ...
                    const messageResult = await sk.invokeAsync(
                        ...
                        'ChatMemorySkill',
                        'GetAllChatMessages',
                        await AuthHelper.getSKaSAccessToken(instance)
                    );
